...
async def add_attachment(file, dirpath, attachment_path):
    ...
    return file
async def remove_audio(file, dirpath):
    LOGGER.info(f"Removing audio from file: {file}")
    temp_file = f"{file}.temp.mkv"
    full_file_path = os.path.join(dirpath, file)
    temp_file_path = os.path.join(dirpath, temp_file)
    ...
    if process.returncode != 0:
        err = stderr.decode().strip()
        LOGGER.error(err)
        LOGGER.error(f"Error removing audio from file: {file}")
        return file

    os.replace(temp_file_path, full_file_path)
    LOGGER.info(f"Audio removed successfully from file: {file}")
    return file

async def swap_audio(file, dirpath, new_audio_path):
    LOGGER.info(f"Swapping audio in file: {file}")
    temp_file = f"{file}.temp.mkv"
    full_file_path = os.path.join(dirpath, file)
    temp_file_path = os.path.join(dirpath, temp_file)
    ...
    cmd = [
        ...
        new_audio_path,
        "-map",
        "0",        # Keep all streams from the original file
        "-map",
        "-0:a",     # Remove all original audio streams
        "-map",
        "1:a",      # Add new audio from the second input
        "-c",
        "copy",
        temp_file_path,
    ]
    ...
    if process.returncode != 0:
        err = stderr.decode().strip()
        LOGGER.error(err)
        LOGGER.error(f"Error swapping audio in file: {file}")
        return file
